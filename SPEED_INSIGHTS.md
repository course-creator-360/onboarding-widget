...
For Node.js backend applications, Speed Insights tracking is typically implemented on the frontend. If you're serving HTML from your Express server, add the tracking script to your HTML templates:
...
Add these scripts before the closing `</body>` tag in your HTML templates.
...
For versions of Next.js older than 13.5, import the `<SpeedInsights>` component from `@vercel/speed-insights/react` and create a dedicated client component:
...
**TypeScript/JavaScript (src/App.vue)**
```vue
<script setup>
...
**TypeScript/JavaScript (layouts/default.vue)**
```vue
<script setup>
import { SpeedInsights } from '@vercel/speed-insights/vue';
</script>
...
**Astro (BaseHead.astro with speedInsightsBeforeSend)**
```astro
---
import SpeedInsights from '@vercel/speed-insights/astro';
const { title, description } = Astro.props;
...
For generic JavaScript frameworks, import the `injectSpeedInsights` function from the package. **This should only be called once in your app, and must run in the client**.
